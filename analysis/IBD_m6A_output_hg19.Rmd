...
```{r echo=FALSE}
ctwas_gene_res %>% 
  select(c(genename, region_tag, susie_pip, z)) %>% 
  arrange(desc(susie_pip)) %>% slice_head(n=20)
```
...
ctwas_gene_res %>% filter(type == "gene") %>%
...
top_ctwas_gene_res <- lapply(split(ctwas_gene_res, ctwas_gene_res$group), function(i){
  head(i[order(-i$susie_pip),c("genename","susie_pip","group","region_tag")], max(sum(i$susie_pip >0.5)), 20)
})
...
ctwas_gene_res %>% filter(group=="m6AQTL") %>% 
  select(c(genename, region_tag, susie_pip, z)) %>% arrange(desc(susie_pip)) %>% slice_head(n=10)
```
...
ctwas_gene_res %>% filter(group =="sQTL") %>% 
  select(c(peak_id, genename, pos, region_tag, susie_pip, z)) %>% 
...
#genes with PIP>0.8 or 20 highest PIPs
head(df_gene, max(sum(df_gene$combined_pip>0.8), 40))
```
...
```{r}
tbl <- read.table("~/projects/m6A_in_disease_genetics/data/nasser_2021_ABC_IBD_genes.txt", skip=15, sep='\t', header=T)
df_gene %>% filter(genename %in% intersect(df_gene$genename, tbl$KnownGene))
```

```{r}
m6a_twas<-read.csv("~/projects/m6A_in_disease_genetics/data/m6A_TWAS_results.csv")
m6a_twas<-m6a_twas %>% filter(Trait == "Inflammatory Bowel Disease" )
colnames(m6a_twas)[2]<-"genename"
m6a_twas<-left_join(m6a_twas, df_gene[, c("genename", "m6A_pip")])
print("The causal genes identified by both methods:")
m6a_twas %>% arrange(desc(m6A_pip)) %>% filter(m6A_pip > 0.5) %>%
  dplyr::select(genename, m6A.PEAK.ID, TWAS.P.Bonferroni, m6A_pip)
print("The causal genes specifically identified by ctwas:")
df_gene %>% filter(m6A_pip > 0.5)
...
```{r}
eg="GLRX"
df_gene[df_gene$genename==eg, ]

a <- locus_plot(region_tag=df_gene[df_gene$genename==eg, "region_tag"], 
                      return_table=T,
                      focus=NULL,
                      label_genes=NULL,
                      rerun_ctwas=F,
...
```{r}
make_track_plots <- function(region_tag, 
                             extend.size=0,
                             start = NULL,
                             end = NULL,
                             full.mode = TRUE,
  ...
  gene.query<-GRanges(seqnames = paste0("chr", unique(a$chrom)),
                      IRanges(start = min(a$pos),
                              end = max(a$pos)
                      ))
  
  # build GRanges for each track
  m6a <- a %>% filter(a$group=="m6AQTL")
  m6a_peaks<-GRanges(m6a$peak_id)
  ...
  ## gene tracks
  grtrack<-build_grtrack(curr.locus.gr = gene.query,
                            start = start(gene.query),
                            end = end(gene.query),
                            genome = "hg19")
  ...
  if(to.highligh){
    ht1 <- HighlightTrack(trackList,
                          ...
                          col = 'pink')
    dist.full <- unique(dsRNA.locus$physical.pos) - end(gene_query)
    dist.short <- unique(dsRNA.locus$physical.pos) - end(m6a_dsRNA_gr)
    
    if(dist.full > extend.size|dist.short > extend.size){
       print(
         sprintf("Extend the window size to be longer than %d for 
                 full mode and %d for short mode.", 
                 dist.full, dist.short)
         )
       }
  }else{
    ht1 <- trackList
