---
title: "cTWAS_analysis_for_white_blood_counts"
author: "Jing Gu"
date: "`r Sys.Date()`"
output:
  html_document:
...
results_dir <- paste0("/project2/xinhe/jinggu/projects/m6a_genetics/ctwas/output")
...
outs<-list()
for(model in c("top1", "enet", "lasso")){
...
```{r}
ctwas_gene_res<-joint_outs$top1$df_genes
ctwas_snp_res<-joint_outs$top1$df_snps
...
ctwas_gene_E_res <- ctwas_gene_res %>% filter(type=="eQTL")
ctwas_gene_E_res[,"ensembl_gene_id"]<-
  unlist(lapply(ctwas_gene_E_res$gene_id, function(i){strsplit(i, "[.]")[[1]][1]}))
ctwas_gene_E_res <- left_join(ctwas_gene_E_res, unique(G_list %>% select(c(hgnc_symbol, ensembl_gene_id))))

ctwas_gene_S_res <- ctwas_gene_res %>% filter(type=="sQTL")
...
G_list_genes <- unique(G_list %>% select(hgnc_symbol, chromosome_name,
                                         ensembl_gene_id,
...
gene_gr<-GRanges(seqnames = paste0("chr", G_list_genes$chromosome_name),
                 ...
                 hgnc_symbol=G_list_genes$hgnc_symbol
                 )
#df<-mergeByOverlaps(splicing_gr, gene_gr)
nearest<-distanceToNearest(splicing_gr, gene_gr)
ctwas_gene_S_res$hgnc_symbol <- gene_gr[nearest@to, ]$hgnc_symbol


ctwas_gene_m6A_res <- ctwas_gene_res %>% filter(type=="m6AQTL")
ctwas_gene_m6A_res[,c("peak_id", "hgnc_symbol")]<-
...
ctwas_snp_res$hgnc_symbol=NA
cols<-setdiff(intersect(colnames(ctwas_gene_res), colnames(ctwas_snp_res)), "id")
ctwas_res <- rbind(ctwas_gene_res[, cols],
                   ctwas_snp_res[, cols])
saveRDS(ctwas_res, paste0(results_dir, "/", analysis_id, "/", analysis_id, "_ctwas_full_res.RDS"))
```
...
top_ctwas_gene_res <- lapply(split(ctwas_gene_res, ctwas_gene_res$type), function(i){
  head(i[order(-i$susie_pip),c("hgnc_symbol","susie_pip","type","region_tag")], max(sum(i$susie_pip >0.6)), 20)
})
#genes with PIP>0.8 or 20 highest PIPs
#head(ctwas_gene_res[order(-ctwas_gene_res$susie_pip),c("hgnc_symbol","susie_pip","type","region_tag")], max(sum(ctwas_gene_res$susie_pip>0.8), 20))
...
```{r echo=FALSE}
ctwas_gene_res %>% filter(type=="m6AQTL") %>% 
  select(c(hgnc_symbol, region_tag, susie_pip, z)) %>% arrange(desc(susie_pip)) %>% slice_head(n=20)
```
...
m6a_res <-ctwas_gene_res %>% filter(type=="m6AQTL") %>% 
  group_by(hgnc_symbol) %>% summarise(total_susie_pip=sum(susie_pip)) %>% 
...
```{r echo=FALSE}
ctwas_gene_res %>% filter(type=="sQTL") %>% 
  select(c(peak_id, hgnc_symbol, region_tag, susie_pip, z)) %>% arrange(desc(susie_pip)) %>% slice_head(n=20)
```
...
```{r}
ctwas_gene_res %>% filter(type=="sQTL" & !is.na(hgnc_symbol)) %>% 
  group_by(hgnc_symbol) %>% summarise(total_susie_pip=sum(susie_pip)) %>% arrange(desc(total_susie_pip)) %>% slice_head(n=10)
```
...
df_gene <- aggregate(ctwas_gene_res$susie_pip, by=list(ctwas_gene_res$hgnc_symbol), FUN=sum)
colnames(df_gene) <- c("hgnc_symbol", "combined_pip")
df_gene$expression_pip <-0
df_gene$splicing_pip <-0
df_gene$m6A_pip <-0

#drop duplicated gene names
df_gene$expression_pip<- ctwas_gene_E_res[sapply(df_gene$hgnc_symbol, match, ctwas_gene_E_res$hgnc_symbol), "susie_pip"]
df_gene$expression_pip[is.na(df_gene$expression_pip)]<-0
df_gene$splicing_pip<- ctwas_gene_S_res[sapply(df_gene$hgnc_symbol, match, ctwas_gene_S_res$hgnc_symbol), "susie_pip"]
df_gene$splicing_pip[is.na(df_gene$splicing_pip)]<-0
df_gene$m6A_pip<- ctwas_gene_m6A_res[sapply(df_gene$hgnc_symbol, match, ctwas_gene_m6A_res$hgnc_symbol), "susie_pip"]
df_gene$m6A_pip[is.na(df_gene$m6A_pip)]<-0

df_gene$region_tag <- ctwas_gene_res[sapply(df_gene$hgnc_symbol, match, ctwas_gene_res$hgnc_symbol), "region_tag"]
