...
```{r}
tbl <- read.csv("~/projects/m6A_in_disease_genetics/data/zhao_silver_genes.csv")
df_gene <- left_join(df_gene, tbl %>% dplyr::select(c(genename, annotation)), by="genename")

print("Table of combined PIPs for LCL silver standard genes:")
df_gene %>% filter(genename %in% tbl[tbl$annotation=="known", ]$genename)

print("Table of combined PIPs for LCL bystander genes:")
df_gene %>% filter(genename %in% tbl[tbl$annotation=="bystander", ]$genename) %>% slice_head(n=10)

print("Overlaps with previously identified high PIP genes that are either silver standard or bystander genes:")
sub_tbl<-tbl %>% filter(!is.na(PIP) & PIP >0.8 & annotation %in% c("known", "bystander")) 
df_gene %>% filter(genename %in% sub_tbl$genename)
